# ------------------------------------------------------------------------------------------
# Description: This function was lifted from the agricolae package version 1.3.3 which 
#              will be used to generate randomization for Alpha Lattice Design.
# Source: agricolae 1.3.3
# ------------------------------------------------------------------------------------------
#' @name design.alpha
...
design.alpha <- function (trt, k, r, serie = 2, seed = 0, kinds = "Super-Duper", 
                          randomization = TRUE) {
  number <- 10
  if (serie > 0) 
    number <- 10^serie
  ...
  if (ntr%%k != 0) 
    ...
    if (serie == "") {
      ...
      cat("\nReplication:", r, "\n")
      cat("\nEfficiency factor\n(E )", E, "\n\n<<< Book >>>\n")
      parameters <- list(design = "alpha", trt = trt, 
