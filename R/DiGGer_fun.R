...
#' @title desPlot  
#' 
#' @description Plot a matrix representing an experimental design to the current graphics device.
#' @param dsgn A matrix representation of an experimental design.
#' @param trts Specific treatment labels to be plotted.
#' @param rprop Proportion of the row boundary of an experimental unit to plot.
#' @param cprop Proportion of the column boundary of an experimental unit to plot.
#' @param label If TRUE print a label for all treatments or specific treatments listed in trts. If FALSE labels are not printed
#' @param plotchar A single text character or plotting symbol to be used as the label.
#' @param plotbndry If plotbndry is TRUE, a polygon is used to plot the experimental unit.
#' @param chtdiv A reduction factor for the plotting symbol.
#' @param bdef A matrix rx2. Each row defines the rows and columns in a block template to be plotted.
#' @param bcol The colour used for the block boundary.
#' @param bwd The width of the block boundary to be plotted.
#' @param rotate If TRUE the matrix is rotated 90 degrees for plotting.
#' @param new If TRUE a new plot is produced, if FALSE the current plot is added to.
#' @param cstr String to label Columns of the matrix.
#' @param rstr String to label Rows of the matrix.
#' @param rlab If TRUE rows of the design are labelled.
#' @param clab If TRUE columns of the design are labelled.
#' @param ... Other arguments to polygon used for plotting experimental units in the design.
#' @source DiGGer 1.0.5
# ------------------------------------------------------------------------------------------

desPlot <- function (dsgn, trts = NULL, rprop = 1, cprop = 1, label = TRUE, 
                     plotchar = NULL, plotbndry = TRUE, chtdiv = 2, bdef = NULL, 
                     bcol = 1, bwd = 2, rotate = FALSE, new = TRUE, cstr = "Range", 
                     rstr = "Row", rlab = TRUE, clab = TRUE, ...) 
{
  charot <- 0
  if (rotate) {
    dc <- dim(dsgn)[2]
  ...
  if (new) {
    ...
    if (rotate) {
      if (rlab) {
        mtext(paste(rev(seq(nr))), side = 2, line = 0, 
              at = -seq(nr), cex = cexval, adj = 1, las = 1)
      }
      mtext(rstr, side = 2, line = 1, at = -nr/2 - 1/2, 
            adj = 0.5)
      mtext(cstr, side = 3, line = 1, at = nc/2 + 1/2, 
            adj = 0.5)
      if (clab) {
        mtext(paste(seq(nc)), side = 3, line = 0, at = seq(nc), 
              cex = cexval)
      }
    }
    else {
      if (rlab) {
        mtext(paste(seq(nr)), side = 2, line = 0, at = -seq(nr), 
  ...
  if (is.null(trts)) {
    if (label) {
      if (!is.null(plotchar)) {
        points(drange, drow, pch = plotchar)
      }
      else {
  ...
  if (!is.null(bdef)) {
    ...
    for (i in seq(dims[1])) {
      nri <- bdef[i, 1]
      nci <- bdef[i, 2]
      for (j in seq(ceiling(nr/nri))) {
        for (k in seq(ceiling(nc/nci))) {
          xoff <- nci * (k - 1)
          yoff <- nri * (j - 1)
          ncimod <- nci
          nrimod <- nri
          ...
          lines(xi + xoff + c(1, 1, ncimod, ncimod, 
                              1), yi - yoff - c(1, nrimod, nrimod, 1, 
                                                1), lwd = bwd, col = bcol)
        }
      }
    }
